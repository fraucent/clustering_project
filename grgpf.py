...
from tfidfStorage import TFIDF_reader, tfidf_cosine_distance
import numpy
...
class GRGPF(object):
    ...
    class Cluster(object):
        ...
        def _init_merge_plain(cls, cluster1, cluster2):
            ...
            for idx in range(0, len(all_points_1)):
                all_points_1[idx] = (all_points_1[idx][0],
                                     all_points_1[idx][1] +
                                     cluster1.N*(self.compute_distance(cluster2.clustroid, all_points_1[idx][0])**2 + distance_c1_c2) +
                                     cluster2.clustroid_rowsum)
            for idx in range(0, len(all_points_2)):
                all_points_2[idx] = (all_points_2[idx][0],
                                     all_points_2[idx][1] +
                                     cluster2.N * (self.compute_distance(cluster1.clustroid, all_points_2[idx][0]) ** 2 + distance_c1_c2) +
            ...
            all_points = [(x[0], x[1], self.compute_distance(self.clustroid, x[0])) for x in all_points if x[0] != self.clustroid]
        ...
        def _recompute(self, points):
            ...
            :return:
            """
            dists = numpy.zeros((len(points), len(points)))
        ...
        def add_point(self, new_point):
            ...
            if len(self.points) >= 2*self.k+1:
                ...
                distance_nearest = [(self.compute_distance(self.nearest[i], self.clustroid)**2, i) for i in range(0, self.k)]   #should be sorted by
                                                                                                                               # invariant
                distance_furthest = [(self.compute_distance(self.furthest[i], self.clustroid)**2, i) for i in range(0, self.k)] #should be sorted by
                                                                                                                               # invariant
                ...
                self.nearest_rowsums = [(self.nearest_rowsums[i] if i != -1 else new_point_rowsum) for _, i in distance_nearest]
                self.furthest = [(self.furthest[i] if i != -1 else new_point) for _, i in distance_furthest]
                self.furthest_rowsums = [(self.furthest_rowsums[i] if i != -1 else new_point_rowsum) for _, i in distance_furthest]
                ...
                if(min_p != -1):
                    ...
                    all_distinct_points = {x for x in [i[0] for i in self.nearest] + [i[0] for i in self.furthest] + [new_point[0],self.clustroid[0]]}
                    distinct_points = []
                    for pt, rowsum in [(self.clustroid, self.clustroid_rowsum),(new_point, new_point_rowsum)]+\
                    ...
                    self.furthest = [x[0] for x in reversed(distinct_points[len(distinct_points) - self.k:len(distinct_points)])]
                    self.furthest_rowsums = [x[1] for x in reversed(distinct_points[len(distinct_points) - self.k:len(distinct_points)])]
                else:
